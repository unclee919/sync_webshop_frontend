  ...
  return <div className={`collection-storyteller ${isRtl ? 'rtl' : 'ltr'}`} style={{ '--collection-accent': collection.accent_color || '#E6B85C' }}>
    ...
    <main id="editorial-story" className="editorial-main container">
      {(collection.sections || []).map((section, index) => {
        ...
        if (layout === 'Gallery') return <motion.section className="editorial-section editorial-gallery" key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}><div className="editorial-copy">{eyebrow && <span className="editorial-kicker">{eyebrow}</span>}{sectionTitle && <h2>{sectionTitle}</h2>}{bodyMarkup(body)}</div><div className="editorial-gallery-grid">{[image, image2].filter(Boolean).map((src, imageIndex) => <img key={src} src={src} alt={`${sectionTitle || title} ${imageIndex + 1}`} loading="lazy" />)}</div></motion.section>
