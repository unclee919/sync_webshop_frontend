...
export default function FitGuide({ item, settings = {} }) {
  ...
  return <>
    <button type="button" className="fit-guide-trigger" onClick={() => setOpen(true)} data-magnetic="true"><span>⌁</span><span><strong>{t(settings.fit_guide_title_en || 'Find your best fit', settings.fit_guide_title_ar || 'اعثر على المقاس المناسب')}</strong><small>{t('A private, on-device guide', 'دليل خاص يعمل على جهازك')}</small></span><b>↗</b></button>
