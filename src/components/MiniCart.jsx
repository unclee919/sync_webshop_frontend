...
export default function MiniCart({ open, onClose }) {
  ...
  return (
    <>
      {open && <button type="button" className="mini-cart-backdrop" aria-label={isArabic ? 'إغلاق السلة' : 'Close cart'} onClick={onClose} />}
      <aside className={`mini-cart ${open ? 'is-open' : ''} ${isRtl ? 'rtl' : 'ltr'}`} aria-hidden={!open}>
        ...
        {hasThreshold && <div className="mini-cart-progress" aria-label={isArabic ? 'تقدم الشحن المجاني' : 'Free shipping progress'}>
